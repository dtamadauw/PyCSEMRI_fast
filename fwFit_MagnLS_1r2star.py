...
def fwFit_MagnLS_1r2star(imDataParams, algoParams, initParams):
	...
	#For initParams
	water_r_init = np.abs(initParams['amp_w']).flatten(order='F').astype(np.float64)
	water_i_init = np.zeros(img_dim[0]*img_dim[1], dtype=np.float64)
	fat_r_init = np.abs(initParams['amp_f']).flatten(order='F').astype(np.float64)
	fat_i_init = np.zeros(img_dim[0]*img_dim[1], dtype=np.float64)
	r2s_init = initParams['r2starmap'].flatten(order='F').astype(np.float64)
	...
	libuwwfs.fwFit_MagnLS_1r2star_c.argtypes = [ctypes.POINTER(ImDataParams_str), ctypes.POINTER(AlgoParams_str), ctypes.POINTER(InitParams_str), ctypes.POINTER(OutParams_str)]
	libuwwfs.fwFit_MagnLS_1r2star_c.restype = None
	libuwwfs.fwFit_MagnLS_1r2star_c(ctypes.byref(imDataParams_c), ctypes.byref(algoParams_c), ctypes.byref(initParams_c), ctypes.byref(outParams_c))
	
	outParams = {
        'water_amp': np.reshape(wat_r_amp + 1j*wat_i_amp, (im_dim[0],im_dim[1])),
		'fat_amp':  np.reshape(fat_r_amp + 1j*fat_i_amp, (im_dim[0],im_dim[1])),
        'r2starmap':  np.reshape(r2starmap, (im_dim[0],im_dim[1])),
        'fm':  np.reshape(fm, (im_dim[0],im_dim[1]))
	}
